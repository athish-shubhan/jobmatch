...
//  LOGIN
// ═══════════════════════════════════════════════════════════════
function LoginPage({ setAuth, setPage }) {
...
function AIAdvisor({ call }) {
  ...
  const runAI = async () => {
    if (!GEMINI_API_KEY) { setResponse("⚠ Add your Gemini API key in jobmatch/.env"); return; }
    setLoad(true); setResponse("");
    try {
      const [profile, mySkills] = await Promise.all([call("/applicants/me"), call("/applicants/me/skills")]);
      const job = jobs.find(j => j.job_id===parseInt(jobId));
      const prompt = `You are a career advisor.\n\nApplicant: ${profile.name}, ${profile.experience_years} years experience\nSkills: ${mySkills.map(s => s.skill_name).join(", ")}\n\nJob: ${job.title} at ${job.company_name}\nDescription: ${job.description}\n\nGive short, specific career advice for this match.`;
      const res = await fetch(
        `https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key=${GEMINI_API_KEY}`,
        { method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ contents:[{ parts:[{ text:prompt }] }] }) }
      );
      const data = await res.json();
      if (!res.ok) { setResponse("Gemini Error: "+(data.error?.message||"Unknown")); return; }
      setResponse(data.candidates?.[0]?.content?.parts?.[0]?.text || "No response from Gemini.");
  ...
  return (
    <div>
      ...
      <p style={S.muted}>Profile fetched from MySQL via Express → sent to Gemini 2.5 Flash</p>
      {err && <ErrBox msg={err} />}
      <div style={{ ...S.card, marginBottom:16, borderColor:"#4f46e544" }}>
        <div style={{ fontSize:12, color:"#475569", lineHeight:1.7 }}>
          Calls <strong style={{ color:"#a5b4fc" }}>GET /api/applicants/me</strong> + <strong style={{ color:"#a5b4fc" }}>GET /api/applicants/me/skills</strong> from MySQL, then sends to <strong style={{ color:"#86efac" }}>Gemini 2.5 Flash</strong>.
        </div>
      </div>
...
function TableViewer({ call, authUser }) {
